...
public class DefaultImapService implements ImapService {
    ...
    private boolean metadataEnvelope;

    private boolean metadataBody;
    ...
    private static Logger LOG = Logger.getLogger(DefaultImapService.class.getName());
    ...
    public void setFetchMetadataEnvelope(boolean metadataEnvelope) {
        this.metadataEnvelope = metadataEnvelope;
    }
    ...
    private synchronized void autoConnect() throws IOException, ImapException {
        if (mAutoconnect && !mIoService.isConnected()) {
            ...
            if (LOG.isLoggable(Level.FINE)) {
                LOG.fine("IMAP Server Welcome message : " + welcome);
            }
            login();
        }
    }
    ...
    // return mSelectedFolderStatus != null ? mSelectedFolderStatus.getFolderName() : null;
    // }
    ...
    public synchronized ImapMessageMetadata fetchMessageMetadataById(int id) throws IOException,
        ...
        String macro = "FAST";
        if (metadataEnvelope & !metadataBody)
            macro = "ALL";
        else if (!metadataEnvelope & metadataBody)
    ...
    private synchronized Part readPart(String a) throws IOException, ImapException {
        ...
        if (LOG.isLoggable(Level.FINE)) {
            LOG.fine("Reading " + size + " characters");
        }
        StringBuilder sb = new StringBuilder();
