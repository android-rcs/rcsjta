...
public class ContactsListActivity extends ListActivity implements
    ...
    /**
     * An approximation of the background color of the pinned header. This color is used when the
     * pinned header is being pushed up. At that point the header "fades away". Rather than
     * computing a faded bitmap based on the 9-patch normally used for the background, we will use a
     * solid color, which will provide better performance and reduced complexity.
     */
    ...
    /**
     * Return the selection arguments for a default query based on the {@link #mDisplayOnlyPhones}
     * flag.
     */
    ...
    /**
     * A {@link TextHighlightingAnimation} that redraws just the contact display name in a list
     * item.
     */
        ...
        Cursor c = getContentResolver()
                .query(dataUri,
                        new String[] {
                                ...
                                RawContacts.ACCOUNT_TYPE, Phone.TYPE,
                                Phone.LABEL
    ...
    private final class ContactItemListAdapter extends CursorAdapter implements
        ...
        /**
         * Callback on the UI thread when the content observer on the backing cursor fires. Instead
         * of calling requery we need to do an async query so that the requery doesn't block the UI
                    ...
                    while (phonesCursor.moveToNext()) {
                        ...
                        try {
                            ...
                            if (items.contains(phone)) {
                                // found joyn contact
                                view.setPresence(getResources().getDrawable(
        ...
        /**
         * Run the query on a helper thread. Beware that this code does not run on the main UI
         * thread!
         */
        @Override
        ...
        /**
         * Computes the state of the pinned header. It can be invisible, fully visible or partially
         * pushed up out of the view.
         */
        ...
        /**
         * Configures the pinned header by setting the appropriate text label and also adjusting
         * color if necessary. The color needs to be adjusted when the pinned header is being pushed
         * up from the view.
         */
    ...
    /**
     * Build the {@link Uri} for the given {@link ListView} position, which can be used as result
     * when in {@link #MODE_MASK_PICKER} mode.
     */
